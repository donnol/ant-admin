...
export default class StandardForm extends React.Component {
  ...
  renderFormItem = () => {
    ...
    for (let i in columns) {
      ...
      let wrapperCol;
      let allCol;
      if (!singleColumn.labelCol && !singleColumn.wrapperCol) {
      ...
      allCol =
        singleColumn.labelCol.span +
        singleColumn.labelCol.offset +
        singleColumn.wrapperCol.span +
        singleColumn.wrapperCol.offset;
      labelCol = {
      ...
      formItem.push(
        <Col span={allCol} key={singleColumn.dataIndex}>
          <FormItem
            labelCol={labelCol}
            wrapperCol={wrapperCol}
            label={singleColumn.title}
          >
            {getFieldDecorator(singleColumn.dataIndex, {
              rules: singleColumn.rules
            })(singleColumn.render())}
          </FormItem>
        </Col>
      );
  ...
  render = () => {
    return (
      <Form onSubmit={this.handleSubmit}>
        <Row>{this.renderFormItem()}</Row>
        <Row>{this.renderButton()}</Row>
      </Form>
    );
  };
}
