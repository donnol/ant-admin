...
export default class PageHeader extends React.Component {
  getDropdownMenu = () => {
    ...
    return (
      <div>
        <List className={styles.list} locale={{ emptyText: "暂无提醒消息" }}>
          {items}
        </List>
      </div>
    );
          ...
          {menu ? (
            ...
            >
              <div className={styles.notify}>
                <Badge dot={this.props.notice.length != 0 ? true : false}>
                  <Icon type="bell" />
                </Badge>
              </div>
            </Dropdown>
          ) : null}
          <div className={styles.reload} onClick={this.props.onReload}>
            <Icon type="reload" />
          </div>
        </div>
