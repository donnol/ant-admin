...
export default class MyTreeList extends React.Component {
  ...
  renderNode = root => {
    ...
    if (root.id == "_all") {
      ...
      if (childNodes.length == 0) {
        return (
          <TreeNode title={renderNode(root.data)} key={root.id} isLeaf={true} />
        );
      } else {
        return (
  ...
  filterNode = (root, filter) => {
    ...
    if (root.id != "_all") {
      let filterNodeHandler =
        this.props.filterNode ||
        function(input, data) {
    ...
    if (newChildren.length == 0 && root.id != "_all") {
      return null;
    } else {
      return {
        id: root.id,
  ...
  render = () => {
    ...
    return (
      <div
        ...
        className={classname(style.container, this.props.className)}
