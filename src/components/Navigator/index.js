...
export default class Navigator extends React.Component {
  ...
  getMainMenuInner = (menus, key) => {
    ...
    for (let i in menus) {
      ...
      if (children.length == 0) {
        item = (
          <Menu.Item key={key + i} path={menu.path}>
  ...
  getDropdownMenu = () => {
    ...
    for (let i in this.props.login.dropdown) {
      ...
      menuItem.push(
        <Menu.Item key={i} onClick={singleDropdown.onClick}>
          {singleDropdown.name}
        </Menu.Item>
      );
    }
    return <Menu onSelect={this.onSelectDropDown}>{menuItem}</Menu>;
  };
  render = () => {
    return (
        ...
        >
          <Dropdown overlay={this.getDropdownMenu()}>
            <div className={style.head}>
              <Avatar icon="user" className={style.icon} />
              <div className={style.user}>{this.props.login.name}</div>
            </div>
          </Dropdown>
          {this.getMainMenu()}
        </Sider>
